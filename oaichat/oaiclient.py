...
class OaiClient:
    ...
    def __init__(self, name='OaiClient', user=None):
        ...
        print("Done." if handshake.get('handshake') ==
              'ok' else "Unexpected response '%s'" % handshake)
...
if __name__ == '__main__':
    client = OaiClient(('Your name is Pepper.',
                       'We are currently at the TechEdge Studio in Oslo, Norway.', 'You are a robot.'))
    while True:
        s = raw_input('> ')
        if s:
