...
# seconds, maximum recording time, also default value for startRecording(), Google Speech API only accepts up to about 10-15 seconds
RECORDING_DURATION = 15
# seconds, for auto-detect mode: amount of seconds before the threshold trigger that will be included in the request
LOOKAHEAD_DURATION = 1.0
# seconds, for auto-detect mode: idle time (RMS below threshold) after which we stop recording and recognize
IDLE_RELEASE_TIME = 200.0
# IDLE_RELEASE_TIME = 2.0
# seconds, minimum recording time after we started recording (autodetection)
HOLD_TIME = 3.0
# Hz, be careful changing this, both google and Naoqi have requirements!
SAMPLE_RATE = 48000

# seconds, timespan during which calibration is performed (summing up RMS values and calculating mean)
CALIBRATION_DURATION = 4
# factor the calculated mean RMS gets multiplied by to determine the auto detection threshold (after calibration)
...
# Fills pre-buffer with last samples when stopping recording. WARNING: has performance issues!
...
class SpeechRecognitionModule(ALModule):
    ...
    def __init__(self, moduleName, naoIp, naoPort=9559):
        try:
            ...
            # self.memory = naoqi.ALProxy("ALMemory", naoIp, naoPort)
            ...
            # TODO: find a default value that works fine so we don't need to calibrate every time
            ...
            self.buffer = []
            self.preBuffer = []
            # length in samples (len(self.preBuffer) just counts entries)
    ...
    def start(self):
        ...
        print("INF: SpeechRecognitionModule: starting!. name: %s    time: %s" %
              (self.getName, current_datetime))
        ...
        nNbrChannelFlag = 0
        nDeinterleave = 0
        # setting same as default generate a bug !?!
        audio.setClientPreferences(
            self.getName(),  SAMPLE_RATE, nNbrChannelFlag, nDeinterleave)
        audio.subscribe(self.getName())
        # GetName() = SpeechRecognition
    ...
    def processRemote(self, nbOfChannels, nbrOfSamplesByChannel, aTimeStamp, buffer):
        ...
        try:
            ...
            aSoundData = np.reshape(
                aSoundDataInterlaced, (nbOfChannels, nbrOfSamplesByChannel), 'F')
            ...
            if (self.isCalibrating or self.isAutoDetectionEnabled or self.isRecording):
                ...
                rmsMicFront = self.calcRMSLevel(
                    self.convertStr2SignedInt(aSoundData[0]))
                ...
                if (rmsMicFront >= self.autoDetectionThreshold):
                    ...
                    if (self.isAutoDetectionEnabled and not self.isRecording and not self.isCalibrating):
                        self.startRecording()
                        print("threshold surpassed: " + str(rmsMicFront) +
                              " more than " + str(self.autoDetectionThreshold))
            ...
            if (not self.isCalibrating):
                if (self.isRecording):
                    ...
                    overshoot = (self.preBufferLength -
    ...
    def stopCalibration(self):
        ...
        self.autoDetectionThreshold = CALIBRATION_THRESHOLD_FACTOR * \
            (self.rmsSum / self.framesCount)
